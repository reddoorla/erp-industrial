...
>
	...
	>
		{#if slice.variation === 'embed'}
			<ContentWidth
				class="text-center h-12 md:h-56 flex flex-col justify-center items-center overflow-hidden py-10 md:pb-0"
			>
					...
					<div class="flex flex-col md:flex-row gap-4" transition:fly={{ y: 20, delay: 300 }}>
						...
						{#if slice.variation === 'teams'}
							...
							>
								...
								{#each slice.items as item, i (i)}
									<div
										class="md:w-1/3 pr-8 flex flex-col gap-8 md:items-start md:justify-end shrink-0"
									>
					...
					{#key activeOverlay}
						<div class="h-full w-full" out:fade in:fade={{ delay: 300 }}>
							...
							>
								...
								<div
									class="min-h-96 my-8 md:w-1/2 text-white flex flex-col overflow-y-scroll items-end"
								>
									<PrismicRichText field={slice.items[activeOverlay].body_text} />
								</div>
